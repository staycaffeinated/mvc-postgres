...
class RootExceptionHandlingIT extends AbstractIntegrationTest {
	...
	class ExceptionTests {
		...
		/**
		 * This exercises the GlobalExceptionHandler by simulating an
		 * HttpRequestMethodNotSupportedException. If properly configured, the
		 * ProblemHandling method that handles this exception should catch it and
		 * configure a proper problem/json response. The point of this test is not to
		 * validate ProblemHandling, but to ensure these kinds of exceptions return a
		...
		void onHttpRequestMethodNotSupported() throws Exception {
			...
			// Note: Mockito won't allow its <code>thenThrows</code> method to throw this
			// particular exception.
			// When attempted, Mockito gives the error "Checked exception is invalid for
			// this method".
